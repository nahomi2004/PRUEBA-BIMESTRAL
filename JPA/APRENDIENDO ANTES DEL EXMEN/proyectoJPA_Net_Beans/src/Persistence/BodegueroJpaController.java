...
public class BodegueroJpaController implements Serializable {
    ...
    public void create(Bodeguero bodeguero) throws PreexistingEntityException, Exception {
        if (bodeguero.getPaquetes() == null) {
            bodeguero.setPaquetes(new ArrayList<Paquete>());
        }
        ...
        try {
            ...
            for (Paquete paquetesPaqueteToAttach : bodeguero.getPaquetes()) {
            ...
            bodeguero.setPaquetes(attachedPaquetes);
            em.persist(bodeguero);
            for (Paquete paquetesPaquete : bodeguero.getPaquetes()) {
                Bodeguero oldBodegueroOfPaquetesPaquete = paquetesPaquete.getBodeguero();
                paquetesPaquete.setBodeguero(bodeguero);
                paquetesPaquete = em.merge(paquetesPaquete);
                if (oldBodegueroOfPaquetesPaquete != null) {
                    oldBodegueroOfPaquetesPaquete.getPaquetes().remove(paquetesPaquete);
                    oldBodegueroOfPaquetesPaquete = em.merge(oldBodegueroOfPaquetesPaquete);
                }
            }
    ...
    public void edit(Bodeguero bodeguero) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Bodeguero persistentBodeguero = em.find(Bodeguero.class, bodeguero.getCedula());
            List<Paquete> paquetesOld = persistentBodeguero.getPaquetes();
            List<Paquete> paquetesNew = bodeguero.getPaquetes();
            ...
            for (Paquete paquetesNewPaquete : paquetesNew) {
                if (!paquetesOld.contains(paquetesNewPaquete)) {
                    Bodeguero oldBodegueroOfPaquetesNewPaquete = paquetesNewPaquete.getBodeguero();
                    paquetesNewPaquete.setBodeguero(bodeguero);
                    paquetesNewPaquete = em.merge(paquetesNewPaquete);
                    if (oldBodegueroOfPaquetesNewPaquete != null && !oldBodegueroOfPaquetesNewPaquete.equals(bodeguero)) {
                        oldBodegueroOfPaquetesNewPaquete.getPaquetes().remove(paquetesNewPaquete);
                        oldBodegueroOfPaquetesNewPaquete = em.merge(oldBodegueroOfPaquetesNewPaquete);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(String id) throws NonexistentEntityException {
        ...
        try {
            ...
            List<Paquete> paquetes = bodeguero.getPaquetes();
            for (Paquete paquetesPaquete : paquetes) {
                paquetesPaquete.setBodeguero(null);
                paquetesPaquete = em.merge(paquetesPaquete);
            }
            em.remove(bodeguero);
            em.getTransaction().commit();
